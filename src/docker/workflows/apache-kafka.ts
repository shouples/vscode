...
export const CONTAINER_NAME_PREFIX = "vscode-kafka-broker";

export class ApacheKafkaWorkflow extends LocalResourceWorkflow {
  resourceKind: string = "Kafka";
  static imageRepo = "apache/kafka";

  logger = new Logger("docker.workflow.apache-kafka");

  // ensure only one instance of this workflow can run at a time
  private static instance: ApacheKafkaWorkflow;
  private constructor() {
    super(); // must call super() in derived class constructor
  }
  static getInstance(): ApacheKafkaWorkflow {
    if (!ApacheKafkaWorkflow.instance) {
      ApacheKafkaWorkflow.instance = new ApacheKafkaWorkflow();
    }
    return ApacheKafkaWorkflow.instance;
  }

  /**
   * Start `apache/kafka` resources locally:
  ...
  async stop(
    ...
    const repoTag = `${ApacheKafkaWorkflow.imageRepo}:${this.imageTag}`;
    const containerListRequest: ContainerListRequest = {
      filters: JSON.stringify({
        ancestor: [repoTag],
